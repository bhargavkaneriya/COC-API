...
router.post("/create", async (req, res) => {
  try {
    ...
    const response = await requestHandler.createRequest(req.body);
...
router.get("/list", async (req, res) => {
  try {
    if (!(req.query.customer_id || req.query.dealer_id)) {
      jsonResponse(
        res,
        responseCodes.BadRequest,
        errors(labels.LBL_MISSING_PARAMETERS["EN"], responseCodes.BadRequest),
    ...
    const response = await requestHandler.requestList(req.query);
    jsonResponse(res, responseCodes.OK, null, response);
  } catch (error) {
    console.log("error", error);
    try {
...
router.get("/request-details", async (req, res) => {
  try {
    ...
    const response = await requestHandler.requestDetails(req.query);
    jsonResponse(res, responseCodes.OK, null, response);
  } catch (error) {
    console.log("error", error);
    try {
